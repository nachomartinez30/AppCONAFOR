...
public class FrmSitioRevision extends javax.swing.JInternalFrame {
    ...
    public void revisarSitio(Integer upmID) {
        this.upmID = upmID;
        cmbUPM.setSelectedItem(this.upmID);
    ...
    private boolean validarCamposObligatoriosSitioAccesible() {
        if (cmbUPM.getSelectedItem() == null) {
            ...
            txtEPE.requestFocus();
            return false;
        } else if (!rbtEvidenciaSi.isSelected() && !rbtEvidenciaNo.isSelected()) {
            JOptionPane.showMessageDialog(null,
                    "Error! Debe seleccionar si hubo evidencia de muestreo ",
                    "Sitio", JOptionPane.INFORMATION_MESSAGE);
            rbtEvidenciaSi.requestFocus();
            return false;
        } else {
            return true;
        }
    ...
    private boolean validarCamposObligatoriosSitioInaccesible() {
     ...
     private boolean validarDatosCoordenadas() {
        ...
        int errorPresicion = Integer.parseInt(txtEPE.getText());

        if (vc.sonGradosLatitud(gradosLatitud, "Sitio")) {
            txtGradosLatitud.requestFocus();
            txtGradosLatitud.setText("");
            return false;
        } else if (vc.sonMinutos(minutosLatitud, "Sitio")) {
            txtMinutosLatitud.requestFocus();
            txtMinutosLatitud.setText("");
            return false;
        } else if (vc.sonSegundos(segundosLatitud, "Sitio")) {
            txtSegundosLatitud.requestFocus();
            txtSegundosLatitud.setText("");
            return false;
        } else if (vc.sonGradosLongitud(gradosLongitud, "Sitio")) {
            txtGradosLongitud.requestFocus();
            txtGradosLongitud.setText("");
            return false;
        } else if (vc.sonMinutos(minutosLongitud, "Sitio")) {
            txtMinutosLongitud.requestFocus();
            txtMinutosLongitud.setText("");
            return false;
        } else if (vc.sonSegundos(segundosLongitud, "Sitio")) {
            txtSegundosLongitud.requestFocus();
            txtSegundosLongitud.setText("");
            return false;
    ...
    private void initComponents() {
        ...
        jPanel11.add(LblEPE, new org.netbeans.lib.awtextra.AbsoluteConstraints(80, 0, 80, -1));

        txtDatum.setHorizontalAlignment(javax.swing.JTextField.CENTER);
        txtDatum.setText("WGS_84");
        txtDatum.setEnabled(false);
        jPanel11.add(txtDatum, new org.netbeans.lib.awtextra.AbsoluteConstraints(0, 20, 80, -1));
        ...
        jPanel11.add(LblEvidencia, new org.netbeans.lib.awtextra.AbsoluteConstraints(160, 0, 170, -1));
        ...
        rbtEvidenciaSi.setEnabled(false);
        rbtEvidenciaSi.setNextFocusableComponent(rbtEvidenciaNo);
        jPanel11.add(rbtEvidenciaSi, new org.netbeans.lib.awtextra.AbsoluteConstraints(220, 20, -1, -1));
        ...
        jPanel11.add(rbtEvidenciaNo, new org.netbeans.lib.awtextra.AbsoluteConstraints(260, 20, -1, -1));
        ...
        jPanel11.add(txtEPE, new org.netbeans.lib.awtextra.AbsoluteConstraints(80, 20, 80, -1));
    ...
    private void chkAccesibleActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_chkAccesibleActionPerformed
        if (chkAccesible.isSelected()) {
            ...
            if (respuesta == JOptionPane.YES_OPTION) {
                this.cdSitio.deleteSitio(this.upmID, sitio);
            }
        }
        combo.reiniciarComboModel(cmbSitio);
        fillSitiosRevision();
    ...
    private void cmbSitioActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmbSitioActionPerformed
        if (cmbSitio.getSelectedItem() != null) {
            Integer sitio = (Integer) cmbSitio.getSelectedItem();
            this.sitioID = this.cdSitio.getSitioID(this.upmID, sitio);
